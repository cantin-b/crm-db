...
-- IMPORTANT : on ne touche pas aux versions existantes.
-- On crée une V024 qui définit proprement la RPC.

-- Drop si présent (quel que soit ce qui existe)
drop function if exists public.staging_promote_batch(text);

-- Recreate avec la signature attendue par ton front (ordre: batch_id, inserted_count)
...
declare
  ...
  v_cnt int;
begin
  -- 1) créer un lot pour ce label (sans helper : full inline)
  insert into public.list_batches(label)
  values (p_batch_label)
  returning id into v_batch_id;

  -- 2) promotion staging -> prospects
  ...
  select
    ...
    s.phone,  -- on laisse tel quel pour compat immédiate

    -- parse num tolérant "2 300,50", espaces, € etc.
    ...
    -- enums (stricts et alignés à ta DB)
    case
    ...
    -- GEO : priorité CP (départements IDF), sinon lookup ville unique via geo_city_index, sinon NULL
    (
      case
  ...
  -- Retour aligné: (batch_id, inserted_count)
